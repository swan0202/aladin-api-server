...
# Render Dashboard → Environment → Environment Variables에 설정:
# Key: ALADIN_TTB_KEY
# Value: 본인의 알라딘 TTB 키
...
def aladin_api_get(endpoint: str, params: dict, timeout: int = 15) -> dict:
    """
    알라딘 API 호출 공통 함수.
    실패를 빈 배열로 숨기지 않고 명확한 502 오류로 반환합니다.
    """
    require_ttb_key()
    ...
    if response.status_code != 200:
        ...
        print("응답 일부:", response.text[:1000])
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "hint": (
                    "403이면 Render 서버 IP가 CloudFront에서 차단되었거나 "
                    "TTB 키 설정/호출 제한 문제일 수 있습니다."
                ),
            },
        )
...
def ttb_search_proxy(
    ...
    """
    이전 프론트엔드 호환용 검색 엔드포인트.
    /api/search 사용을 권장합니다.
    """
    data = aladin_api_get(
        "ItemSearch.aspx",
        {
...
def resolve_aladin_item_id(lookup_id: str) -> str:
    """
    ISBN을 알라딘 ItemId로 변환합니다.
    이미 ItemId인 경우 그대로 반환합니다.
    """
    if not lookup_id:
        return ""
...
def ttb_lookup_proxy(
    ...
    """
    알라딘 상세 정보 조회 + API에 없는 소개글/문구 보완.
    """
    data = aladin_api_get(
        "ItemLookUp.aspx",
        {
...
def extract_image_from_node(node) -> Optional[str]:
    ...
    for image in node.select("img"):
        ...
        candidates.extend(
            re.findall(r"url\(([^)]+)\)", image_style, re.IGNORECASE)
        )

    for child in node.select("[style]"):
        child_style = child.get("style") or ""
        candidates.extend(
            re.findall(r"url\(([^)]+)\)", child_style, re.IGNORECASE)
        )
...
def classify_aladin_image_url(src: str) -> tuple[Optional[str], Optional[str]]:
    ...
    if (
        "/spine/" in lower
        or "/spineflip/" in lower
        or re.search(r"_(d|s|sl)\.(jpg|png)$", lower)
    ):
    ...
    if (
        re.search(r"_(fl|f|wfl|1|2)\.(jpg|png)$", lower)
        or "/cover500/" in lower
        or "/cover/" in lower
    ):
...
def scan_aladin_image_urls(html: str) -> tuple[dict, list[str]]:
    ...
    urls = re.findall(
        r'(?:https?:)?//image\.aladin\.co\.kr/product/[^"\'\s>)]+(?:\.jpg|\.png)',
        html,
        re.IGNORECASE,
    )
...
def get_book_images(
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail={
                "message": "알라딘 상품 페이지를 불러오지 못했습니다.",
                "reason": str(error),
            },
        )
    ...
    if not all(
        [images["front"], images["spine"], images["back"]]
    ):
    ...
    for src in all_urls:
        ...
        if "cover" in normalized_src.lower() and re.search(
            r"_\d\.", normalized_src
        ):
    ...
    for src in all_urls:
        ...
        if (
            ...
            not images["spine"]
            and re.search(
                r"(/spine/|/spineflip/|_(d|s|sl)\.jpg)$",
                normalized_src,
                re.IGNORECASE,
            )
        ):
